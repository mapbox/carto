...
tree.Filterset.prototype.checkConflict = function(other) {
    var err;
    for (var id in other.filters) {
        err = this.conflict(other.filters[id]);
        if (err) return err;
    }
    return false;
};
...
// Returns whether the new filter constitutes a conflict
//
// [foo='bar'][foo='foo']
// [foo='bar'][foo!='bar']
tree.Filterset.prototype.conflict = function(filter) {
    ...
    if (filter.op === '=' && key + '=' in this.filters &&
        ...
        value == this.filters[key + '!='].val.toString())) {
        return {
            message: filter.toString() + ' added to ' + this.toString() + ' produces an invalid filter'
        };
    }
...
// false
//
// null means that it will have no affect.
tree.Filterset.prototype.addable = function(filter) {
    ...
    switch (filter.op) {
        ...
        case '!=':
            if (key + '=' in this.filters) return null;
