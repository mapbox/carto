...
tree.functions = {
    ...
    mix: function (color1, color2, weight) {
        ...
        var rgb1 = _.map(chroma(color1.toString()).rgb(), normalize);
        var rgb2 = _.map(chroma(color2.toString()).rgb(), normalize);
        ...
        var mix = null;
        if (perceptual) {
            mix = husl.fromRGB.apply(this, rgb);
            mix[1] = mix[1] / 100;
            mix[2] = mix[2] / 100;
        }
        else {
            mix = chroma(_.map(rgb, denormalize)).hsl();
        }
