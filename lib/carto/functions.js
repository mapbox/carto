...
tree.functions = {
    ...
    desaturatep: function (color, amount) {
        if (!('getPerceptual' in color)) return null;
    ...
    mix: function (color1, color2, weight) {
        ...
        if (comp1.perceptual && !comp2.perceptual) {
            comp2 = comp2.toPerceptual().getComponents();
        }
        else if (!comp1.perceptual && comp2.perceptual) {
            comp1 = comp1.toPerceptual().getComponents();
        }

        var w1 = (((w * a == -1) ? w : (w + a) / (1 + w * a)) + 1) / 2.0;
        var w2 = 1 - w1;

        var hsl = [comp1[0] * w1 + comp2[0] * w2,
                   comp1[1] * w1 + comp2[1] * w2,
                   comp1[2] * w1 + comp2[2] * w2];
    ...
    grayscalep: function (color, amount) {
        if (!('getPerceptual' in color)) return null;
        return this.grayscale(color.toPerceptual(), amount);
    },
